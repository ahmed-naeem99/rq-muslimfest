...
export async function POST(request: Request) {
  try {
    ...
    // Insert into game_data table - FIXED
    await sql`
    INSERT INTO game_data (user_id, level, score, inventory)
    VALUES (${userId}, ${1}, ${0}, ${JSON.stringify({})})
    `;

    // Insert mission 1 into missions table
    await sql`
    INSERT INTO missions (user_id, mission)
    VALUES (${userId}, ${1})
    `;
    ...
    await sql`
    INSERT INTO team_members (team_id, member_name, member_email, user_id)
    VALUES (${userId}, ${teamMembers[0].name}, ${email}, ${userId})
    `;
